...
# Response label prefix used in the summary CSVs (binary classification metrics
# are stored as response_<metric>, e.g. response_auc).
...
def plot_sensitivity(agg, figures_dir=FIGURES_DIR, metric=DEFAULT_METRIC):
    ...
    for param, sub in agg.groupby("param"):
        ...
        fig.tight_layout()
        fig.savefig(os.path.join(figures_dir, f"sensitivity_{param}_{metric_label}.pdf"))
        plt.close(fig)
