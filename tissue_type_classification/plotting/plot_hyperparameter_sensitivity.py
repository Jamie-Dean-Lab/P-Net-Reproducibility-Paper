...
Tissue-type classification is a multiclass task, so sensitivity here is
characterised with the validation weighted F1 (the model selection metric, see
val_metric in the config) rather than the regression R^2 used for
radiosensitivity_prediction.
"""
...
# Friendly y-axis labels per metric.
...
def load_val_results(run_dir=RUN_DIR, metric=DEFAULT_METRIC):
    ...
    # The tissue-type pipeline scores with task="group", so the summary CSVs store
    # each metric under its bare name (e.g. "f1"), without a per-label prefix.
    col = metric
    rows = []
...
def plot_sensitivity(agg, figures_dir=FIGURES_DIR, metric=DEFAULT_METRIC):
    ...
    file_token = metric_label.replace(" ", "_")
    fontsize = 18
    ...
    for param, sub in agg.groupby("param"):
        ...
        fig.tight_layout()
        fig.savefig(os.path.join(figures_dir, f"sensitivity_{param}_{file_token}.png"), dpi=300)
        plt.close(fig)
